...
```{r, echo=FALSE}
Pereira$cost_day <- round(Pereira$cost_day,1)
names(Pereira) = c("Año-mes", "Grupo-Demo", "Sexo", "Costo diario", "Dieta", "Mes", "Año")


Pereira$`Costo diario` <- as.character(Pereira$`Costo diario`)
Pereira$Año <- as.character(Pereira$Año)
Pereira$Mes <- as.character(Pereira$Mes)
Pereira$Sexo <- as.character(Pereira$Sexo)




datatable(Pereira[, 2:7], 
          filter = 'top', 
          options = list(
...
file_path <- "txt-imputacion/output_Pereira.txt"
...
exclude_words <- c("Pereira", "Average daily cost per",
...
"cogote", "murillo", "paletero", "bota", "Riboflavin", "bolsita", "Color")
