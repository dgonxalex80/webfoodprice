...
```{r, echo=FALSE}
Ibague$cost_day <- round(Ibague$cost_day,1)
names(Ibague) = c("Año-mes", "Grupo-Demo", "Sexo", "Costo diario", "Dieta", "Mes", "Año")


Ibague$`Costo diario` <- as.character(Ibague$`Costo diario`)
Ibague$Año <- as.character(Ibague$Año)
Ibague$Mes <- as.character(Ibague$Mes)
Ibague$Sexo <- as.character(Ibague$Sexo)



datatable(Ibague[, 2:7], 
          filter = 'top', 
          options = list(
...
file_path <- "txt-imputacion/output_Ibague.txt"
...
exclude_words <- c("Ibague", "Average daily cost per",
