...
```{r, echo=FALSE}
Cartagena$cost_day <- round(Cartagena$cost_day,1)
names(Cartagena) = c("Año-mes", "Grupo-Demo", "Sexo", "Costo diario", "Dieta", "Mes", "Año")


Cartagena$`Costo diario` <- as.character(Cartagena$`Costo diario`)
Cartagena$Año <- as.character(Cartagena$Año)
Cartagena$Mes <- as.character(Cartagena$Mes)
Cartagena$Sexo <- as.character(Cartagena$Sexo)




datatable(Cartagena[, 2:7], 
          filter = 'top', 
          options = list(
...
file_path <- "txt-imputacion/output_Cartagena.txt"
...
exclude_words <- c("Cartagena", "Average daily cost per",
