...
```{r, echo=FALSE}
Villavicencio$cost_day <- round(Villavicencio$cost_day,1)
names(Villavicencio) = c("Año-mes", "Grupo-Demo", "Sexo", "Costo diario", "Dieta", "Mes", "Año")

Villavicencio$`Costo diario` <- as.character(Villavicencio$`Costo diario`)
Villavicencio$Año <- as.character(Villavicencio$Año)
Villavicencio$Mes <- as.character(Villavicencio$Mes)
Villavicencio$Sexo <- as.character(Villavicencio$Sexo)

datatable(Villavicencio[, 2:7],
  filter = 'top',
  options = list(
    pageLength = 22,
    autoWidth = TRUE,
...
file_path <- "txt-imputacion/output_Villavicencio.txt"
...
exclude_words <- c("Cucuta", "Average daily cost per",
  ...
  "Thiamine", "to input them", "use the optional parameter called", "VitaminA", "VitaminC", "Zinc","Calcium", "Folate","Lipids", "Phosphorus", "roja", "tacos", 
"cogote", "murillo", "paletero", "bota", "Riboflavin")
