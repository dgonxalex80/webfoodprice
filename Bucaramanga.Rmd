...
```{r, echo=FALSE}
Bucaramanga$cost_day <- round(Bucaramanga$cost_day,1)
names(Bucaramanga) = c("Año-mes", "Grupo-Demo", "Sexo", "Costo diario", "Dieta", "Mes", "Año")


Bucaramanga$`Costo diario` <- as.character(Bucaramanga$`Costo diario`)
Bucaramanga$Año <- as.character(Bucaramanga$Año)
Bucaramanga$Mes <- as.character(Bucaramanga$Mes)
Bucaramanga$Sexo <- as.character(Bucaramanga$Sexo)

datatable(Bucaramanga[, 2:7], 
          filter = 'top', 
          options = list(
...
file_path <- "txt-imputacion/output_Bucaramanga.txt"
...
exclude_words <- c("Bucaramanga",
  ...
  "Average daily cost per", "Thiamine", "to input them", "use the optional parameter called", "VitaminA", "VitaminC", "Zinc","Calcium", "tacos", "Cali","Folate","Lipids", "Phosphorus","Riboflavin","Calabacín", "bota", "cogote", "paletero"
)
